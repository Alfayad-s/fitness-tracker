...
type ExerciseImportSaveButtonProps = {
  extraction: ExerciseImportExtraction;
  saved?: boolean;
  onSaved: (savedContent: string) => void;
};

export function ExerciseImportSaveButton({
  extraction,
  saved = false,
  onSaved,
}: ExerciseImportSaveButtonProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch("/api/ai/exercise-import-save", {
        method: "POST",
